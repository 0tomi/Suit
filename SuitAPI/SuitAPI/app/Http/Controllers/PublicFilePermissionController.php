...
class PublicFilePermissionController extends Controller
{
    ...
    /**
     * Get the permissions of the current user on a public file.
     */
    public function myPermissions(Request $request, $id)
    {
        $publicFile = PublicFile::findOrFail($id);
        $user = $request->user();

        return response()->json([
            'can_update' => $user->can('update', $publicFile),
            'can_delete' => $user->can('delete', $publicFile),
        ]);
    }
